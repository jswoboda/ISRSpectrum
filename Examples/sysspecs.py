...
def main():
    ...
    Cia = np.sqrt(spconst.k * (Te + 3.0 * Ti) / 16.0 / spconst.m_p)

    # make list of dictionaries for the radar systems
    biglist = [
        {"name": "AMISR", "Fo": 449e6, "Fs": 50e3, "alpha": 70.0},
        {"name": "Sondrestrom", "Fo": 1290e6, "Fs": 100e3, "alpha": 80.0},
        {"name": "Haystack", "Fo": 440e6, "Fs": 50e3, "alpha": 65.0},
        {"name": "Arecibo", "Fo": 430e6, "Fs": 50e3, "alpha": 45.0},
        {"name": "Jicamarca", "Fo": 50e6, "Fs": 10e3, "alpha": 1.0},
    ]
    ...
    for ima, idict in enumerate(biglist):
        curax = axvec[ima]
        k = 2.0 * np.pi * 2 * idict["Fo"] / spconst.c
        xloc = np.array([-k * Cia, k * Cia]) / 2 / np.pi / 2
        ISS1 = ISSnew.Specinit(
            centerFrequency=idict["Fo"],
            bMag=0.4e-4,
            nspec=512,
            sampfreq=idict["Fs"],
            dFlag=True,
        )
        (omeg, spec) = ISS1.getspecsep(
            databloc, species, vel=0.0, alphadeg=idict["alpha"], rcsflag=False
        )

        lines[0] = curax.plot(omeg * 1e-3, spec, label="Output", linewidth=5)[0]
        lines[1] = curax.stem(
            xloc * 1e-3,
            np.ones(2) * np.amax(spec),
            linefmt="g--",
            markerfmt="go",
            basefmt=" ",
        )[0]
    ...
    figmplf.suptitle(
        r"Spectrums $N_e$ = {0:.1e}m$^{{-3}}$, $T_e$ = {1:.0f}$^o$K, $T_i$ = {2:.0f}$^o$K".format(
            Ne, Te, Ti
        ),
        fontsize=20,
    )
