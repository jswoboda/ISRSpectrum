...
def main():
    ...
    for ima,idict in enumerate(biglist):
        ...
        ISS1 = ISSnew.Specinit(centerFrequency = idict['Fo'], bMag = 0.4e-4, nspec=256, sampfreq=idict['Fs'],dFlag=True)
        ...
        lines[1] = curax.stem(xloc*1e-3, np.ones(2)*np.amax(spec), linefmt='g--', markerfmt='go', basefmt=' ')[0]
